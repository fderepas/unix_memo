...
```
Where ```/root/reboot_if_needed.sh``` is a script which reboots with a random delay, if file ```/run/reboot-required``` exists. The reboot is performed in the upcoming hour:
